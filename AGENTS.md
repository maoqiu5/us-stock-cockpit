...
- `/root/apps/portal/docs/BRIANHUB_GATEWAY_AND_SSO.md`
    ...
    - us-stock-cockpit is the BrianHub US-stock cockpit for real market data, holdings discipline, candidates, strategy/backtest work, after-hours review, and local data assets. It must not use simulated data for production decisions.
    - Gateway owns TLS/Caddy/public routing. US-stock data belongs under `/root/apps/us-stock-cockpit/data/usstock/`, with SQLite primary DB and JSON mirrors/backups. Future broker API integrations still require human confirmation, not automatic live trading.
